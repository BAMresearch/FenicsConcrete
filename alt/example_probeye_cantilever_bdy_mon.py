...
def forward_model_run(param1, param2, param3, param4, sensors_vec, _flag, ndim=2):
    for i in range(len(sensors_vec)):
        problem.add_sensor(sensors_vec[i])
    ...
    problem.solve() 
    #mt("MCMC run")
    model_data = collect_sensor_solutions(problem.sensors, len(sensors_vec))
    problem.delete_sensor()
    if _flag==0:
        return model_data[:,0]
    elif _flag==1:
...
class FwdModel1(ForwardModelBase):
    def interface(self):
        ...
        self.input_sensors = [Sensor("sensing_positions"), Sensor("flag")] #sensor provides a way for forward model to interact with experimental data.
    ...
    def response(self, inp: dict) -> dict:    #forward model evaluation
        ...
        t = inp["k_x"]
        u = inp["k_y"]
        v = inp["sensing_positions"]
        w = inp["flag"]
        displacement_results = forward_model_run(m, b, t, u, v, w)
        return {"disp" : displacement_results}

ProbeyeProblem.add_forward_model(FwdModel1("CantileverModel1"), experiments=["Test1","Test2","Test3","Test4"]) #
#ProbeyeProblem.add_forward_model(FwdModel1("CantileverModel1"), experiments=["Test1"]) #,"Test2","Test3","Test4"
#ProbeyeProblem.add_forward_model(FwdModel1("CantileverModel2"), experiments=["Test2"])
#ProbeyeProblem.add_forward_model(FwdModel1("CantileverModel3"), experiments=["Test3"])
#ProbeyeProblem.add_forward_model(FwdModel1("CantileverModel4"), experiments=["Test4"])
