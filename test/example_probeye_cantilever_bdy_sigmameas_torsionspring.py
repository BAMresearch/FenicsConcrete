...
distortion_ver_rest = np.random.normal(0, 1e-8, (20))
...
ProbeyeProblem.add_parameter(name = "K_torsion",                     
                            tex=r"$K_{torsion}$",
                            info="Torsional Spring Stiffness",
                            #domain="(0, +oo)",
                            prior=Uniform(low=1e9, high=1e12))
...
class FwdModel1(ForwardModelBase):
    def interface(self):
        self.parameters = ["E", "nu", "K_torsion"]   #E and nu must have been already defined beforehand using add_parameter. # three attributes are must here.
    ...
    def response(self, inp: dict) -> dict:    #forward model evaluation
        ...
        b = inp["nu"]
        t = inp["K_torsion"]
        displacement_results = forward_model_run(m, b, t)
...
true_values = {"E": para['E'], "nu": para['nu'], "K_torsion":para['K_torsion']} 
