...
distortion = np.random.normal(0, 1, (number_of_sensors,2))
#print(distortion) 
...
E_buildup = np.linspace(30,140,30)
...
#fig = plx.line(x=parameters[:,1], y=cost_func_val, labels= {'x':'Youngs Modulus', 'y':'Cost Function'})
#fig = plx.scatter_3d(x=parameters[:,0], y = parameters[:,1] , z=cost_func_val, labels= {'x':'Youngs Modulus', 'y':'Poissons Ratio','z':'Cost Function'})
#fig = go.Figure(data=[go.Mesh3d(x=parameters[:,0], y=parameters[:,1], z=cost_func_val, color='lightpink', opacity=0.50)])
#fig = go.Figure(data=[go.Surface(z=cost_func_val.reshape(30,9), x=E_buildup, y=nu_buildup)])
fig = go.Figure()
...
fig.show()
#plty.title("Cost_func Vs. parameters")
#plty.xlabel("Parameters")
#plty.ylabel("Cost function")
#plty.legend()
#plty.show()
...
#result = optimize.minimize(cost_func, (80, 0.4), args=(displacement_measured),  method='powell') #Newton-CG
