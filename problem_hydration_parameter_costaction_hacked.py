...
class HydrationHeatModel(ForwardModelBase):
    ...
    def response(self, inp: dict) -> dict:
        ...
        heat_list = hydration_fkt(T, time_list, dt, parameter)
...
# max time for synthetic data
tmax = 0
for i in range(len(T_datasets)):
    if tmax < time_data[i][-1]:
        tmax = time_data[i][-1]
...
for i,T in enumerate(T_datasets):
    ...
    hydration_heat_model = HydrationHeatModelSingle()
    ...
    time_data_synth.append([])
    heat_data_synth.append([])

    infered_paramters.append(inference_data.x)
    time_data_synth[i] = plot_time_list
...
# now run the same thing but with synthetic data!!!!
...
# print inferred data
for i in range(len(infered_paramters)):
    print(f'eta: {infered_paramters[i][0]}, B1: {infered_paramters[i][1]}, B2: {infered_paramters[i][2]}')
print('------------')
print(f'eta: {inference_data.x[0]}, B1: {inference_data.x[1]}, B2: {inference_data.x[2]}, E_act: {inference_data.x[3]}')
...
for i,T in enumerate(T_datasets):
    ...
    # get the respective function
    hydration_fkt = material_problem.get_heat_of_hydration_ftk()
    heat_list[i] = hydration_fkt(T, time_list[i], dt, parameter)
