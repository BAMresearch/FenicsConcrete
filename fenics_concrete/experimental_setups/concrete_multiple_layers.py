...
class ConcreteMultipleLayers2DExperiment(Experiment):
    ...
    def setup(self):
        ...
        if self.p.dim == 2:
            self.mesh = df.RectangleMesh(
                df.Point(0.0, 0.0),
                df.Point(self.p.layer_width, self.p.layer_number * self.p.layer_height),
                n_width,
                int(self.p.layer_number) * n_height_layer,
                diagonal="right",
            )
        else:
    ...
    def create_displ_bcs(self, V):
        ...
        if self.p.dim == 2:
            displ_bcs.append(
                df.DirichletBC(V, df.Constant((0, 0)), self.boundary_bottom())
            )
        else:
            raise ValueError("Dimension has to be 2!! for that experiment")
