...
from probeye.definition.distribution import Normal, Uniform, LogNormal, Exponential, MultivariateNormal
...
with open('probabilistic_identification/results_co/test_config.json', 'r') as f: 
...
def prior_func_selection(para :list):
    ...
    elif para[0] == 'MultivariateNormal':
        return MultivariateNormal(mean = para[1]['mu'], cov = para[1]['sigma'])
    elif para[0] == 'LogNormal':
        return LogNormal(mean = para[1]['mu'], std = para[1]['sigma'])
...
parameters_list = ["E", "nu", "G_12", "sigma"] #"E_d",
...
class FEMModel(ForwardModelBase):
    ...
    def response(self, inp: dict) -> dict:    #forward model evaluation
        ...
        if json_object.get('MCMC').get('parameter_scaling') == True:
            problem.E_1.value = (inp["E"][0] + inp["E"][1])*500*10**6   
            problem.E_2.value = (inp["E"][0] - inp["E"][1])*500*10**6   
            ...
            problem.E_1.value = (inp["E"][0] + inp["E"][1])
            problem.E_2.value = (inp["E"][0] - inp["E"][1])
            problem.nu_12.value = inp["nu"]
