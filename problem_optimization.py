...
def tempeature_optimization_1(x, *args):
    ...
    while t <= time:
        ...
        if problem.sensors[1].data[-1][1] > T_limit:
            print(f'Max temperature exceeded! Stopping computation at t={t/60/60}.')
            T_limit_reached = True
            break
        # check for DOH limit
        if problem.sensors[0].data[-1][1] > DOH_min:
            print(f'Min DOH requirement reached! Stopping computation at t={t/60/60}.')
...
def tempeature_optimization_2(x, *args):
    bc_temp = x[0] # boundary temp
...
if setting == 1:
    ...
    result = minimize(tempeature_optimization_1, bounds = bounds_1, args= args_1, workers= -1)
